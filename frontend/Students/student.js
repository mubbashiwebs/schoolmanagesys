...
document.getElementById('admissionForm').addEventListener('submit', async function (e) {
  ...
  if (data.admissionTypes.length <= 0) {
    showToast('error', 'Amission Type is required')
    return
  }

  if (feeStructureSelect.value == '') {
    showToast('error', 'Select Fee Structure')
    return
  }
  if (feeStructureSelect.value == 'default_class_fee') {
  ...
  try {
    ...
    submitBtn.textContent = isEdit ? 'update' : 'submit'
...
async function loadData() {
  ...
  if (studentData.feeStructure) {
    ...
    feeStructureSelect.value = studentData.feeStructure._id
  }
  if (studentData.feeStructure == null) {
    feeStructureSelect.value = 'default_class_fee'
  }
  ...
  if (feeStructureSelect.value != '' && feeStructureSelect.value !== 'default_class_fee') {
    classFeeForFeeStructure = studentData.feeDetails.school.originalFee
  }
  ...
  // show student grnos 
  schoolgrnoInput.value = studentData.grNumbers.school || ''
...
var feeStructures = []
var feeStructureSelect = document.getElementById('feeStructureSelect')
// load fee structures
...
feeStructureSelect.addEventListener('change', () => {
  filterSeletedFeeStructureClassFee()
  updateFee()

})
var classFeeForFeeStructure = null
function filterSeletedFeeStructureClassFee() {
  if (feeStructureSelect.value == '' || feeStructureSelect.value == 'default_class_fee') return
  const selectedId = feeStructureSelect.value;
  const selectedStructure = feeStructures.find(fs => fs._id === selectedId);
  console.log(currentClass)
  const classId = currentClass.value;
  console.log(classId)
  console.log(selectedStructure)
  if (classId) {
    const classFee = selectedStructure.classFees.find(cf => cf.classId._id === classId).Fee
    if (classFee <= 0) {
      alert('No fee defined for this class in selected fee structure')
    }
    console.log(classFee);
    classFeeForFeeStructure = classFee
  }
  else {
    alert('select class and fee structure')
  }
}
...
function updateFee() {
  ...
  if (selected.includes('school') && selectedClass) {
    ...
    console.log('chal rha hon!')
    console.log(feeStructureSelect.value)

    if (feeStructureSelect.value != '' && feeStructureSelect.value !== 'default_class_fee') {
      createFeeSection('School', Number(classFeeForFeeStructure), { discount: studentData?.feeDetails?.school?.discount || 0 }, 'school');
      totalFee += Number(classFeeForFeeStructure);
    }
    else {
