...
export const addSection = async (req, res) => {
  try {
    ...
    res.status(201).json({ data: newSection, message: "Successfully added" });
  } catch (error) {
    console.error("Add Section Error:", error);
    res.status(500).json({ message: "Server error" });
  }
...
export const getSectionsBySchool = async (req, res) => {
  try {
    ...
    if (sections.length > 0) {
      res.status(200).json({ data: sections, message: "Successfully Found" });
    } else {
...
export const updateSection = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, schoolId, campusId } = req.body;
